...
// Función para enviar un mensaje usando la API de CallMeBot a ambos teléfonos
void sendMessage(const String& message) {
  WiFiClient client;
  HTTPClient http;

  // URL y solicitudes para el primer teléfono
  String url1 = "http://api.callmebot.com/whatsapp.php?phone=" + String(telefono1) + "&apikey=" + String(apikey1) + "&text=" + urlEncode(message);
  Serial.println("URL1: " + url1); // Imprime la URL para depuración

  if (WiFi.status() == WL_CONNECTED) {
    http.begin(client, url1);
    http.addHeader("Content-Type", "application/x-www-form-urlencoded");
    int httpResponseCode1 = http.POST("");

    if (httpResponseCode1 > 0) {
      if (httpResponseCode1 == 200) {
        Serial.println("Mensaje enviado exitosamente a telefono 1");
      } else {
        Serial.println("Error al enviar el mensaje a telefono 1");
        Serial.print("Código de respuesta HTTP: ");
        Serial.println(httpResponseCode1);
      }
    } else {
      Serial.print("Error en la solicitud HTTP a telefono 1: ");
      Serial.println(httpResponseCode1);
    }
    http.end();
  } else {
    Serial.println("Conexión WiFi no disponible para telefono 1");
  }

  // URL y solicitudes para el segundo teléfono
  String url2 = "http://api.callmebot.com/whatsapp.php?phone=" + String(telefono2) + "&apikey=" + String(apikey2) + "&text=" + urlEncode(message);
  Serial.println("URL2: " + url2); // Imprime la URL para depuración
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (httpResponseCode2 > 0) {
      if (httpResponseCode2 == 200) {
        Serial.println("Mensaje enviado exitosamente a telefono 2");
      } else {
        Serial.println("Error al enviar el mensaje a telefono 2");
        Serial.print("Código de respuesta HTTP: ");
        Serial.println(httpResponseCode2);
      }
    } else {
      Serial.print("Error en la solicitud HTTP a telefono 2: ");
      Serial.println(httpResponseCode2);
    }
    http.end();
  } else {
    Serial.println("Conexión WiFi no disponible para telefono 2");
  }
}
