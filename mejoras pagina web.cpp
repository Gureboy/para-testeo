...
<body>
  ...
  <div class="container">
    ...
    <button class="boton-volver" onclick="location.href='/'">Volver</button>
  </div>
</body>
</html>
)rawliteral";
...
void handleEliminar() {
  if (server.hasArg("nombre")) {
    String nombreEliminar = server.arg("nombre");
    String nuevoPacientes = "";
    int startIndex = pacientes.indexOf("<tr><td>" + nombreEliminar + "</td>");
    
    if (startIndex != -1) {
      int endIndex = pacientes.indexOf("</tr>", startIndex) + 5;
      pacientes.remove(startIndex, endIndex - startIndex);
    }

    server.sendHeader("Location", "/ver", true);
    server.send(303);
  } else {
    server.send(400, "text/plain", "Nombre del paciente no especificado");
  }
}
...
void handleEnviar() {
  if (server.method() == HTTP_POST) {
    ...
    String apellido = server.arg("apellido");
    String edad = server.arg("edad");
    String ritmoCardiaco = server.arg("ritmoCardiaco");
    String tempCorporal = server.arg("tempCorporal");

    String nuevoPaciente = "<tr><td>" + nombre + "</td><td>" + apellido + "</td><td>" + edad + "</td><td>" + ritmoCardiaco + "</td><td>" + tempCorporal + "</td><td><button onclick=\"location.href='/eliminar?nombre=" + nombre + "'\">Eliminar</button></td></tr>";
