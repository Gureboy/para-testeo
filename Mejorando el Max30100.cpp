...
// Variable para almacenar el último usuario
...
void manejarEnvio() {
    ...
    // Registrar el usuario en el servidor intermediario
    HTTPClient http;
    http.begin(urlServidor);
    http.addHeader("Content-Type", "application/x-www-form-urlencoded");
    String payload = "usuario=" + ultimoUsuario + "&nombre=" + servidor.arg("nombre") + "&ritmoCardiaco=" + String(pacientes[cantidadPacientes - 1].ritmoCardiaco) + "&tempCorporal=" + String(pacientes[cantidadPacientes - 1].tempCorporal);
...
void setup() {
    ...
    // Inicialización del MAX30100
    if (!pox.begin()) {
        Serial.println("ERROR: No se puede encontrar el sensor MAX30100");
        while (1);
    } else {
        Serial.println("Sensor MAX30100 encontrado");
    }
    pox.setOnBeatDetectedCallback(onBeatDetected);
    ticker.attach_ms(50, []() { pox.update(); });

    // Registrar el nombre del usuario al inicio (puedes cambiarlo a través de otro método)
    registrarUsuario("Enfermera/o Maria Villegas");
}
...
void loop() {
    ...
    if (millis() - tsLastReport > REPORTING_PERIOD_MS) {
        ...
        float tempCorporal = 36.5; // Suponiendo una temperatura corporal fija o tomada de otro sensor
        ...
        // Aquí puedes almacenar los datos del sensor en la estructura de paciente si es necesario
        tsLastReport = millis();
    }
}
